...
import { sampleWithRequiredData } from '../advertisement.test-samples';
...
describe('Advertisement Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.advertisements?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to advertisementService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getAdvertisementIdentifier');
      const id = comp.trackId(entity);
      expect(service.getAdvertisementIdentifier).toHaveBeenCalledWith(entity);
